    ...
    return (
      <Flex minH="60vh" align="center" justify="center">
        <VStack gap={3}>
          ...
          <Text color="gray.500">جاري التحميل... Loading quotes...</Text>
        </VStack>
      </Flex>
    );
  }
              ...
              <Flex justify="space-between" align="center" flexWrap="wrap" gap={3}>
                ...
                <VStack align="flex-end" gap={1}>
                  <Badge
                    ...
                    fontWeight="700"
                  >
                    {job.status === "accepted" ? "✓ مقبول Accepted" : 
                     job.status === "quoted" ? "💬 عروض Quoted" : job.status}
                  </Badge>
                  <Text fontSize="xs" color="gray.500">
                    {quotes.length} {quotes.length === 1 ? "عرض Quote" : "عروض Quotes"}
                  </Text>
                </VStack>
              </Flex>
            </Box>
              ...
              <HStack justify="space-between" flexWrap="wrap" gap={3}>
                <Box>
                  <Text fontWeight="800" color="green.700" fontSize="lg">
                    ✓ تم قبول العرض Quote Accepted
                  </Text>
                  <Text fontSize="sm" color="green.600" mt={1}>
                    السعر النهائي: Final Price: {formatGBP(acceptResult.price ?? 0)}
                  </Text>
                </Box>
                <Link
              ...
              {job && (
                ...
                >
                  <HStack justify="space-between" mb={4}>
                    ...
                    >
                      {showDetails ? "إخفاء Hide" : "عرض Show"}
                    </Button>
                  </HStack>
                    ...
                    <VStack gap={3} align="stretch" fontSize="sm">
                      <Box>
                        ...
                        {job.pickupFloor !== null && (
                          <Text fontSize="xs" color="gray.500">
                            الطابق {job.pickupFloor} • {job.pickupHasLift ? "✓ مصعد Lift" : "⚠ بدون مصعد No Lift"}
                          </Text>
                        )}
                      </Box>
                      ...
                      <Box>
                        ...
                        {job.deliveryFloor !== null && (
                          <Text fontSize="xs" color="gray.500">
                            الطابق {job.deliveryFloor} • {job.deliveryHasLift ? "✓ مصعد Lift" : "⚠ بدون مصعد No Lift"}
                          </Text>
                        )}
                      </Box>
              ...
              {items.length > 0 && (
                ...
                >
                  <Text fontSize="lg" fontWeight="800" color="gray.800" mb={3}>
                    📋 العفش Items ({items.reduce((s, i) => s + i.quantity, 0)})
                  </Text>
                    ...
                    {items.map((item) => (
                      ...
                      >
                        ...
                        <Flex gap={2} mt={1} flexWrap="wrap" fontSize="xs" color="gray.500">
                          {item.weightKg && <Text>⚖️ {item.weightKg}kg</Text>}
                          {item.volumeM3 && <Text>📦 {item.volumeM3}m³</Text>}
                          {item.fragile && <Text>⚠️ قابل للكسر Fragile</Text>}
                          {item.requiresDismantling && <Text>🔧 يحتاج فك Dismantling</Text>}
                        </Flex>
                        {item.notes && (
                  ...
                  <Box mt={3} pt={3} borderTop="1px solid" borderColor="gray.200">
                    <HStack justify="space-between" fontSize="sm">
                      <Text color="gray.600">الوزن الكلي Total Weight:</Text>
                      <Text fontWeight="700">{totalWeight.toFixed(1)} kg</Text>
                    </HStack>
                    <HStack justify="space-between" fontSize="sm">
                      <Text color="gray.600">الحجم الكلي Total Volume:</Text>
              ...
              >
                <HStack gap={3} flexWrap="wrap">
                  ...
                  <Box>
                    ...
                    >
                      <option value="price-low">أقل سعر Price: Low → High</option>
                      <option value="price-high">أعلى سعر Price: High → Low</option>
                      <option value="rating">أعلى تقييم Rating: High → Low</option>
                      <option value="recent">الأحدث Most Recent</option>
                    </select>
                  </Box>
                  <Button
                    size="sm"
              ...
              {quotes.length === 0 && (
                ...
                >
                  ...
                  <Text fontSize="sm" color="gray.400" mt={2}>
                    السائقون في منطقتك سيبدأون بإرسال عروضهم قريباً
                  </Text>
                  ...
                  <VStack gap={4} align="stretch">
                      ...
                      <HStack gap={3}>
                        ...
                        <Box>
                          ...
                          <HStack gap={2} mt={1} fontSize="xs" color="gray.500">
                            ...
                            <Text>•</Text>
                            <Text>{q.driver.totalJobs} وظيفة jobs</Text>
                          </HStack>
                        </Box>
                      </HStack>
                        ...
                        {job?.estimatedPrice && (
                          <Text fontSize="xs" color={
                            ...
                            {q.price < job.estimatedPrice 
                              ? `أقل بـ ${formatGBP(job.estimatedPrice - q.price)}`
                              : `أعلى بـ ${formatGBP(q.price - job.estimatedPrice)}`}
                          </Text>
                        )}
                      </VStack>
                    </Flex>
                    ...
                    {q.status === "accepted" ? (
                      ...
                      <Text fontSize="sm" color="orange.600" fontWeight="600">
                        ⚠ هذا السائق لم يكمل إعداد الدفع بعد
                        <br />
                      ...
                      >
                        {accepting === q.id
                          ? "جاري القبول... Accepting..."
                          : `✓ قبول Accept — ${formatGBP(q.price)}`}
                      </Button>
                    ) : null}

                    <Text fontSize="xs" color="gray.400" textAlign="right">
                      تم الإرسال Submitted: {new Date(q.createdAt).toLocaleString("en-GB", {
                        day: "numeric",
                        month: "short",
                        hour: "2-digit",
                        minute: "2-digit",
